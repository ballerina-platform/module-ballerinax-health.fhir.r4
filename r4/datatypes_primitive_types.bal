...
@constraint:String {
    pattern: {
        value: re `\S*`,
        message: "A Uniform Resource Identifier Reference (RFC 3986 ). Note: URIs are case sensitive. For UUID (urn:uuid:53fefa32-fcbb-4ff8-8a92-55ee120877b7), use all lowercase."
    }
}
...
@constraint:String {
    pattern: {
        ...
        message: "A base64Binary is a stream of bytes, base64 encoded (RFC 4648)."
    }
}
...
@constraint:String {
    pattern: {
        ...
        message: "An instant in time in the format YYYY-MM-DDThh:mm:ss.sss+zz:zz (e.g. 2015-02-07T13:28:17.239+02:00 or 2017-01-01T00:00:00Z)."
    }
...
@constraint:String {
    pattern: {
        ...
        message: "It is a date, or partial date (e.g. just year or year + month) as used in human communication. The format is YYYY, YYYY-MM, or YYYY-MM-DD, e.g. 2018, 1973-06, or 1905-08-23. There SHALL be no time zone."
    }
}
...
@constraint:String {
    pattern: {
        value: re `([0-9]([0-9]([0-9][1-9]|[1-9]0)|[1-9]00)|[1-9]000)(-(0[1-9]|1[0-2])(-(0[1-9]|[1-2][0-9]|3[0-1])(T([01][0-9]|2[0-3]):[0-5][0-9]:([0-5][0-9]|60)(\.[0-9]+)?(Z|(\+|-)((0[0-9]|1[0-3]):[0-5][0-9]|14:00)))?)?)?`,
        message: "It is a date, date-time or partial date (e.g. just year or year + month) as used in human communication. The format is YYYY, YYYY-MM, YYYY-MM-DD or YYYY-MM-DDThh:mm:ss+zz:zz, e.g. 2018, 1973-06, 1905-08-23, 2015-02-07T13:28:17-05:00 or 2017-01-01T00:00:00.000Z."
    }
}
...
@constraint:String {
    pattern: {
        ...
        message: "It is a time during the day, in the format hh:mm:ss. There is no date specified."
    }
}
...
@constraint:String {
    pattern: {
        value: re `[^\s]+(\s[^\s]+)*`,
        message: "A code is restricted to a string with at least one character and no leading or trailing whitespace, and there is no whitespace other than single spaces in the contents."
    }
}
...
@constraint:String {
    pattern: {
        ...
        message: "An OID represented as a URI (RFC 3001 ); e.g. urn:oid:1.2.3.4.5"
    }
}
...
@constraint:String {
    pattern: {
        ...
        message: "An id is any combination of upper-case or lower-case ASCII letters ('A'..'Z', and 'a'..'z', numerals ('0'..'9'), '-' and '.', with a length limit of 64 characters."
    }
}
...
@constraint:String {
    pattern: {
        value: re `\s*(\S|\s)*`,
        message: "A markdown is an FHIR string that may contain markdown syntax for optional processing by a markdown presentation engine in the GFM extension of CommonMark format."
    }
}
...
@constraint:Int {
    minValue: {
        value: 1,
        message: "It should be any positive integer."
    }
}
...
@constraint:Int {
    minValue: {
        value: 0,
        message: "It should be any non-negative integer."
    }
}
