...
public const string PROFILE_BASE_PATIENTCORRECTIONCOMMUNICATION = "http://hl7.org/fhir/uv/patient-corrections/StructureDefinition/patient-correction-communication";
public const RESOURCE_NAME_PATIENTCORRECTIONCOMMUNICATION = "Communication";

# FHIR PatientCorrectionCommunication resource record.
#
# + resourceType - The type of the resource describes
# + note - Additional notes or commentary about the communication by the sender, receiver or other interested parties.
# + partOf - The initial communication sent by a Requester for a record correction.
...
# + about - Other resources that pertain to this communication and to which this communication should be associated.
    # * about Slicings
        # 1) PatientCorrectionCommunicationAboutCorrectionTask: Patient Correction Task requesting a correction.
        #       - min = 0
        #       - max = 1
        #
...
@r4:ResourceDefinition {
    ...
    profile: "http://hl7.org/fhir/uv/patient-corrections/StructureDefinition/patient-correction-communication",
    elements: {
        "note" : {
            name: "note",
        ...
        "about" : {
            name: "about",
            dataType: PatientCorrectionTask,
            min: 0,
            max: int:MAX_VALUE,
            isArray: true,
        ...
        "payload" : {
            name: "payload",
            dataType: PatientCorrectionCommunicationPayload,
            min: 0,
            max: int:MAX_VALUE,
            isArray: true,
        ...
        "priority" : {
            name: "priority",
            dataType: PatientCorrectionCommunicationPriority,
            min: 0,
            max: 1,
            isArray: false,
...
public type PatientCorrectionCommunication record {|
    *r4:DomainResource;

    RESOURCE_NAME_PATIENTCORRECTIONCOMMUNICATION resourceType = RESOURCE_NAME_PATIENTCORRECTIONCOMMUNICATION;

    r4:Annotation [] note?;
    @constraint:Array {
        maxLength: {
            value: 1,
            message: "Validation failed for $.Communication.partOf constraint. This field must be an array containing at most one item."
        }
    }
    r4:Reference [] partOf?;
    ...
    PatientCorrectionCommunicationPayload [] payload?;
    @constraint:Array {
        maxLength: {
            value: 1,
            message: "Validation failed for $.Communication.inResponseTo constraint. This field must be an array containing at most one item."
        }
    }
    ...
    PatientCorrectionCommunicationPriority priority?;
    r4:dateTime sent;
    ...
    @constraint:Array {
        minLength: {
            value: 1,
            message: "Validation failed for $.Communication.recipient constraint. This field must be an array containing at least one item."
        }
    }
    ...
    @constraint:Array {
        minLength: {
            value: 1,
            message: "Validation failed for $.Communication.category constraint. This field must be an array containing at least one item."
        },
        maxLength: {
            value: 1,
            message: "Validation failed for $.Communication.category constraint. This field must be an array containing at most one item."
        }
    }
...
# FHIR PatientCorrectionCommunicationPayload datatype record.
#
...
@r4:DataTypeDefinition {
    name: "PatientCorrectionCommunicationPayload",
    baseType: (),
    elements: {
        "extension": {
            name: "extension",
...
public type PatientCorrectionCommunicationPayload record {|
    *r4:BackboneElement;
...
# FHIR PatientCorrectionCommunicationAboutCorrectionTask datatype record.
#

@r4:DataTypeDefinition {
    name: "PatientCorrectionCommunicationAboutCorrectionTask",
    baseType: (),
    elements: {
    },
...
public type PatientCorrectionCommunicationAboutCorrectionTask record {|
    *r4:Reference;

|};


# PatientCorrectionCommunicationPriority enum
public enum PatientCorrectionCommunicationPriority {
