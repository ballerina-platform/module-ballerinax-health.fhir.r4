...
public isolated function compareResources(ResourceCompareRequest resourceCompareRequest) returns error|http:Response {
    http:Response response = new;
    int score = 0;
    foreach string fhirPathRule in resourceCompareRequest.fhirPaths {
        fhirpath:FhirPathResult resultMapSourceResource = fhirpath:getFhirPathResult(<map<json>>resourceCompareRequest.sourceResource.toJson(), fhirPathRule);
        fhirpath:FhirPathResult resultMapTargetResource = fhirpath:getFhirPathResult(<map<json>>resourceCompareRequest.targetResource.toJson(), fhirPathRule);
    ...
    if score == resourceCompareRequest.fhirPaths.length() {
...
public type ResourceCompareRequest record {|
