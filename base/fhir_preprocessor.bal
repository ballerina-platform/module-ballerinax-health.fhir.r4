        ...
        foreach string originalParamName in requestQueryParams.keys() {
            ...
            if searchParamDefinitions.hasKey(queryParam.name) {
                ...
                if self.searchParamConfigMap.hasKey(queryParam.name) && self.searchParamConfigMap.get(queryParam.name).active {
                    ...
                    return createFHIRError(string `Unsupported search parameter : ${queryParam.name}`, ERROR, PROCESSING, 
                ...
                return createFHIRError(string `Unknown search parameter : ${queryParam.name}`, ERROR, PROCESSING,
                                            diagnostic = diagnose, httpStatusCode = http:STATUS_BAD_REQUEST);
            }
...
isolated function processCommonSearchParameter(CommonSearchParameterDefinition definition,
    ...
    if searchParamConfigMap.hasKey(definition.name) {
        ...
        if paramConfig.active {
            ...
            string diagnose = string `Unsupported search parameter \"${queryParam.name}\" for resource type \" 
                ${fhirResourceType}\". Supported search parameters are: ${extractActiveSearchParameterNames(searchParamConfigMap).toBalString()}`;
            return createFHIRError(string `Unsupported search parameter : ${queryParam.name}`, ERROR, PROCESSING_NOT_SUPPORTED, diagnostic = diagnose, 
