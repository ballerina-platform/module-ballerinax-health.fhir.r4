...
# * identifier Slicings
# 1) AUCorePractitionerIdentifierHpii: Healthcare Provider Identifier – Individual (HPI-I)
#       - min = 0
#       - max = 1
#
# + extension - An Extension
# * extension Slicings
# 1) Extension: Gender identity shall be a member of the Gender Identity Response value set if any codes within that value set can apply
#       - min = 0
#       - max = *
#
# 2) Extension: Pronouns shall be a member of the Australian Pronouns value set if any codes within that value set can apply
#       - min = 0
#       - max = *
#
# 3) Extension: Recorded sex or gender type shall be a member of the Common AU Recorded Sex or Gender Type value set if any codes within that value set can apply. Recorded sex or gender source document type shall be a member of the Common AU Recorded Sex or Gender (RSG) Source Document Type value set if any codes within that value set can apply. Recorded sex or gender jurisdiction shall be a member of the Common AU Recorded Sex or Gender (RSG) Source Document Jurisdiction value set if any codes within that value set can apply
#       - min = 0
#       - max = *
#
...
@r4:ResourceDefinition {
    ...
    elements: {
        ...
        "gender" : {
            name: "gender",
            dataType: AUCorePractitionerGender,
            min: 0,
            max: 1,
            isArray: false,
        ...
        "qualification" : {
            name: "qualification",
            dataType: AUCorePractitionerQualification,
            min: 0,
            max: int:MAX_VALUE,
            isArray: true,
        ...
        "name" : {
            name: "name",
            dataType: AUCorePractitionerName,
            min: 1,
            max: int:MAX_VALUE,
            isArray: true,
...
public type AUCorePractitioner record {|
    *r4:DomainResource;

    RESOURCE_NAME_AUCOREPRACTITIONER resourceType = RESOURCE_NAME_AUCOREPRACTITIONER;

    r4:Identifier[]|aubase421:AuPbsprescribernumber[]|aubase421:AuAhpraregistrationnumber[]|aubase421:AuCareagencyemployeeidentifier[]|aubase421:AuHpii[] identifier?;
    r4:Extension[] extension?;
    aubase421:AuAddress[]|r4:Address[] address?;
    AUCorePractitionerGender gender?;
    r4:Extension[] modifierExtension?;
    boolean active?;
    r4:Attachment[] photo?;
    r4:code language?;
    r4:date birthDate?;
    AUCorePractitionerQualification[] qualification?;
    r4:Resource[] contained?;
    r4:Meta meta?;
    @constraint:Array {
       minLength: 1
    }
    AUCorePractitionerName[] name;
...
public enum AUCorePractitionerNameUse {
...
@r4:DataTypeDefinition {
    ...
    elements: {
        ...
        "use": {
            name: "use",
            dataType: AUCorePractitionerNameUse,
            min: 0,
            max: 1,
            isArray: false,
...
public type AUCorePractitionerName record {|
    *r4:HumanName;
...
#
@r4:DataTypeDefinition {
    name: "AUCorePractitionerIdentifierHpii",
    baseType: (),
    elements: {
    },
    serializers: {
...
public type AUCorePractitionerIdentifierHpii record {|
    *r4:Identifier;

|};

# AUCorePractitionerGender enum
public enum AUCorePractitionerGender {
...
# FHIR AUCorePractitionerQualification datatype record.
#
...
@r4:DataTypeDefinition {
    name: "AUCorePractitionerQualification",
    baseType: (),
    elements: {
        "identifier": {
            name: "identifier",
...
public type AUCorePractitionerQualification record {|
    *r4:BackboneElement;
