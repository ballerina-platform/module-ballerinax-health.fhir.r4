...
public isolated function mapCcdaAllergyToFhir(xml actElement) returns r4:AllergyIntolerance? {
    ...
    if isXMLElementNotNull(actElement) {
        ...
        foreach int i in 0 ..< idElement.length() {
            r4:Identifier? mapCcdaIdToFhirIdentifierResult = mapCcdaIdToFhirIdentifier(idElement[i]);
            if mapCcdaIdToFhirIdentifierResult is r4:Identifier {
                allergyIntolerance.identifier[i] = mapCcdaIdToFhirIdentifierResult;
            }
        }
        ...
        if manifestation is r4:CodeableConcept {
            ...
            if typeCode is string && typeCode == "MFST" {
