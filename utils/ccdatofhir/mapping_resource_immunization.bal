    ...
    if isXMLElementNotNull(substanceAdministrationElement) {
        ...
        foreach xml idElem in idElement {
            ...
            if mapCcdaIdToFhirIdentifierResult is r4:Identifier {
                immunization.identifier[index] = mapCcdaIdToFhirIdentifierResult;
                index = index + 1;
            }
        }
        ...
        if lotNumber is string && lotNumber != "" {
        ...
        if code is string {
            if code == "48767-8" {
                ...
                string|error? text = textElement.data();
                if text is string {
...
isolated function mapCcdaStatusCodeToFhirImmunizationStatus(xml codingElement) returns r4:ImmunizationStatus {
    string|error? codeVal = codingElement.data();
    if codeVal !is string {
