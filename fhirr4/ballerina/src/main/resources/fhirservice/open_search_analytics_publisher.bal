// Copyright (c) 2025, WSO2 LLC. (http://www.wso2.com).

// WSO2 LLC. licenses this file to you under the Apache License,
// Version 2.0 (the "License"); you may not use this file except
// in compliance with the License.
// You may obtain a copy of the License at

// http://www.apache.org/licenses/LICENSE-2.0

// Unless required by applicable law or agreed to in writing,
// software distributed under the License is distributed on an
// "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY
// KIND, either express or implied.  See the License for the
// specific language governing permissions and limitations
// under the License.

import ballerina/http;
import ballerinax/health.fhir.r4;
import ballerina/log;
import ballerina/jwt;
import ballerina/time;

# OpenSearchAnalyticsConfig Record.
#
# + enabled - if analytics is enabled or not
# + attributes - the list of attributes picked from x-jwt-assertion to publish
# + url - the URL of the analytics server to publish logs
# + username - the username for the analytics server
# + password - the password for the analytics server
# + moreInfo - configuration for fetching more information
public type OpenSearchAnalyticsConfig readonly & record {|
    string url = "http://localhost:9200/fhirr4/_doc";
    string username?;
    string password?;
    MoreInfoConfig moreInfo = {};
|};

# MoreInfoConfig Record.
#
# + enabled - if more info fetching is enabled or not  
# + url - the URL to fetch more information  
# + username - the username for the more info server  
# + password - the password for the more info server
public type MoreInfoConfig record {|
    boolean enabled = false;
    string url?;
    string username?;
    string password?;
|};

configurable OpenSearchAnalyticsConfig openSearch = {};

const X_JWT_HEADER = "x-jwt-assertion";

isolated service class OpenSearchAnalyticsPublisher {
    *r4:FhirAnalyticsPublisher;

    final string resourceType;
    final http:Client|http:ClientError? logPublisherHttpClient;
    final http:Client|http:ClientError? moreInfoHttpClient;

    public isolated function init(r4:ResourceAPIConfig apiConfig) {
        
        self.resourceType = apiConfig.resourceType;

        final string? analyticsUsername = openSearch?.username;
        final string? analyticsPassword = openSearch?.password;

        // Initialize the log publisher HTTP client
        if analyticsUsername !is () && analyticsPassword !is () {
            self.logPublisherHttpClient = new (openSearch.url, auth = {
                username: analyticsUsername,
                password: analyticsPassword
            });
        } else {
            self.logPublisherHttpClient = new (openSearch.url);
        }

        final string? moreInfoUrl = openSearch.moreInfo?.url;
        final string? moreInfoUsername = openSearch.moreInfo?.username;
        final string? moreInfoPassword = openSearch.moreInfo?.password;

        // Initialize the analytics more info HTTP client
        if !openSearch.moreInfo.enabled || moreInfoUrl is () {
            self.moreInfoHttpClient = ();
        } else if moreInfoUsername !is () && moreInfoPassword !is () {
            self.moreInfoHttpClient = new (moreInfoUrl, auth = {
                username: moreInfoUsername,
                password: moreInfoPassword
            });
        } else {
            self.moreInfoHttpClient = new (moreInfoUrl);
        }
    }

    public isolated function publish(map<string[]> requestHeaders, json|http:ClientError requestPayload, 
                                map<string[]> responseHeaders, json|http:ClientError responsePayload, int statusCode, 
                                string requestPath) returns error? {

        string jwt = requestHeaders.get(X_JWT_HEADER)[0];
        map<string>|error analyticsData = self.constructAnalyticsData(jwt, statusCode);

        if analyticsData is error {
            log:printError(analyticsData.message());
            return;
        }

        if openSearch.moreInfo.enabled {
            json moreInfo = self.getMoreInfo(analyticsData.toJson());
            log:printDebug(`[AnalyticsResponseInterceptor] More info fetched from: ${openSearch.moreInfo.url} [More info]: ${moreInfo.toString()}`);
            foreach var [key, value] in (<map<json>>moreInfo).entries() {
                analyticsData[key] = value.toString();
            }
        }

        http:Client|http:ClientError? logPublisher = self.logPublisherHttpClient;
        log:printDebug(`[AnalyticsResponseInterceptor] Sending log to: ${openSearch.url}`);
        if logPublisher is () || logPublisher is http:ClientError {
            log:printError(`[AnalyticsResponseInterceptor] Failed to create Analytics HTTP client`);
        } else {
            http:Response|http:ClientError result = logPublisher->/.post(analyticsData);
            if result is http:ClientError {
                log:printError(`[AnalyticsResponseInterceptor] Failed publishing log to ${openSearch.url} [Error]: ${result.toString()}`);
            } else if result.statusCode < 200 || result.statusCode >= 300 {
                log:printError(`[AnalyticsResponseInterceptor] Failed publishing log to ${openSearch.url} [Error]: ${result.reasonPhrase}`);
            } else {
                log:printInfo(`[AnalyticsResponseInterceptor] Log published successfully to ${openSearch.url}`);
            }
        }
    }

    # Constructs the analytics data based on the x-jwt-assertion header
    # + jwt - The JWT token from the x-jwt-assertion header
    # + statusCode - The HTTP status code of the response
    # + return - An error if the publishing fails
    isolated function constructAnalyticsData(string jwt, int statusCode) returns map<string>|error {
        [jwt:Header, jwt:Payload]|error decodedJWT = check jwt:decode(jwt);
        if decodedJWT is [jwt:Header, jwt:Payload] {
            [jwt:Header, jwt:Payload] [_, payload] = decodedJWT;

            // Filter logData to include only keys in x_jwt_attributes
            map<string> filteredAnalyticsData = map from string attrKey in analytics.attributes
                where payload[attrKey] !== ()
                select [attrKey, payload[attrKey].toString()];

            filteredAnalyticsData["resourceType"] = self.resourceType;

            // Get more information if configured
            if openSearch.moreInfo.enabled {
                json moreInfo = self.getMoreInfo(filteredAnalyticsData.toJson());
                log:printDebug(`[AnalyticsResponseInterceptor] More info fetched from: ${openSearch.moreInfo.url} [More info]: ${moreInfo.toString()}`);
                foreach var [key, value] in (<map<json>>moreInfo).entries() {
                    filteredAnalyticsData[key] = value.toString();
                }
            }

            filteredAnalyticsData["timestamp"] = time:utcToString(time:utcNow());
            filteredAnalyticsData["statusCode"] = statusCode.toString();
            return filteredAnalyticsData;
        } else {
            log:printError(`[AnalyticsResponseInterceptor] Error decoding x-jwt-assertion header.`);
            return error("[AnalyticsResponseInterceptor] Error decoding x-jwt-assertion header.");
        }
    }

    # Retrieves more information from the configured URL
    #
    # + logData - The log data to send to retrieve more information
    # + return - A JSON object containing additional information
    isolated function getMoreInfo(json logData) returns json {
        http:Client|http:ClientError? moreInfoClient = self.moreInfoHttpClient;
        if moreInfoClient is () || moreInfoClient is http:ClientError {
            log:printError(`[AnalyticsResponseInterceptor] Failed to create More Info HTTP client`);
            return {};
        } else {
            http:Response|http:ClientError result = moreInfoClient->/.post(logData);
            if (result is http:Response) {
                json|error payload = result.getJsonPayload();
                if payload is json {
                    return payload;
                } else {
                    log:printError(`[AnalyticsResponseInterceptor] Failed to extract JSON payload from More Info response.`);
                    return {};
                }
            } else {
                log:printError(`[AnalyticsResponseInterceptor] Failed to fetch more info from ${openSearch.moreInfo.url} [Error]: ${result.toString()}`);
                return {};
            }
        }
    }
}
